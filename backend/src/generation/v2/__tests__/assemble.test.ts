...
describe('buildSummarySlides — Fase 2: Arco de la misión (MISSION_ARC_V2)', () => {
  ...
  describe('Beat de progreso a mitad (eliminado — sesiones breves no lo necesitan)', () => {
    ...
    it('never appears, regardless of the flag or concept count', () => {
      expect(buildSummarySlides(sixKo, sixDistractors, [], [], false).some((s) => s.type === 'motivation')).toBe(false);
      expect(buildSummarySlides(sixKo, sixDistractors, [], [], true).some((s) => s.type === 'motivation')).toBe(false);
    });
  });
});
