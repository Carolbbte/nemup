...
describe('resultsMatch', () => {
  ...
  // model's own re-derivation produces.
  ...
  it('does not match a genuinely different result even when one side is prose-wrapped', () => {
    expect(resultsMatch('10x + 24', '10x + 25')).toBe(false);
  });

  it('does not match a genuinely different result even when one side is prose-wrapped (real case)', () => {
    expect(resultsMatch(
      '10x + 25',
      'Por lo tanto, la expresión que representa la diferencia entre las áreas es (10x + 24) cm².',
    )).toBe(false);
