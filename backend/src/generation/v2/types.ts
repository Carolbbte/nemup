...
export interface KnowledgeConcept {
  ...
  sourceQuote: string;
  /**
   * "procedure" if this concept is a method/operation the student EXECUTES
   * (calculate, solve, reduce, factor, apply a formula); "supporting" if it
   * is vocabulary/definition/taxonomy that prepares for a procedure without
   * being a calculation itself. Always generated by comprehension.ts (see
   * schemas.ts's `required`) — this laxer TS type is optional only for
   * callers constructing a KnowledgeConcept by hand (tests, older data),
   * same convention as `exampleEmoji`. Consumers must treat a missing role
   * as "supporting" (`concept.role ?? 'supporting'`), never as "procedure"
   * — see contentType.ts/assemble.ts for where this is actually consumed,
   * gated behind FEATURE_CONTENT_TYPE_V2.
   */
  role?: 'procedure' | 'supporting';
}
