...
type SummarySlideType = 'concept' | 'key_fact' | 'important' | 'remember' | 'example' | 'curiosity' | 'wow_fact'
  ...
  // Fase 2 (MISSION_ARC_V2, backend) — a single progress beat mid-mission.
...
type Session = {
  ...
  desafio?: { id: string; topic: string; conceptCount: number; slides: any[] };
};
  ...
  return (
    <Pressable onPress={handlePress} style={fcd.container}>
      ...
      <Animated.View style={[fcd.face, fcd.back, backStyle]}>
        ...
        <MathText style={fcd.backText}>{(back)}</MathText>
      </Animated.View>
    </Pressable>
  );
}
...
// ── Mascot-voice feedback for micro_challenge / reinforcement_challenge /
// final_challenge — these already had green/red option coloring but no
// message at all after answering. `wrongAnswerHints`/`hint` were already
// generated by exerciseGenerator.ts and shipped in the slide payload, just
...
const DAILY_MODE_TO_PHASE: Record<DailyMode, 'summary' | 'quiz' | 'flashcards'> = {
  mision: 'summary', quiz: 'quiz', tarjetas: 'flashcards', desafio: 'summary',
};
            ...
            <Pressable onPress={() => router.push('/modals/desafio' as any)} android_ripple={{ color: 'rgba(0,0,0,0.06)' }}>
              {({ pressed }) => (
                <View style={[mds.desafioCard, pressed && { opacity: 0.92 }]}>
                  <View style={mds.desafioCardTop}>
                    <Text style={mds.desafioEmoji}>⚔️</Text>
                    <View style={mds.desafioXpBadge}>
                      <Text style={mds.desafioXpText}>BONUS</Text>
                    </View>
                  </View>
                  <Text style={mds.desafioTitle}>Desafío</Text>
                  <Text style={mds.desafioDesc}>Pon a prueba todos los conceptos</Text>
                  <View style={mds.desafioFoot}>
                    <Text style={mds.desafioDetail}>{session.desafio?.conceptCount} conceptos · {session.desafio?.slides.length} rondas</Text>
                    <View style={mds.desafioArrow}>
      ...
      <View style={{ flex: 1, backgroundColor: BG }}>
        ...
        <SafeAreaView style={{ flex: 1 }} edges={['top']}>
          ...
          >
            ...
            slide?.type === 'quiz' ? (
              <>
                <MathText style={sum.quizQuestionFull}>{slide.question}</MathText>
                  ...
                  {slide.options.map((opt, i) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <MathText style={[sum.quizOptText, showGreen && { color: paletteExtras.verdeTextoOscuro, fontWeight: '700' }, showRed && { color: palette.rojoErrorDark, fontWeight: '700' }]}>
                          {opt.text}
                        </MathText>
                      </Pressable>
                    );
                ...
                {!!slideQuizAnswered && (
                  <View style={[sum.quizFeedback, slideQuizAnswered === slide.correctId ? sum.quizFeedbackOk : sum.quizFeedbackErr]}>
                    ...
                    {!!slide.explanation && <MathText style={sum.quizFeedbackText}>{(slide.explanation)}</MathText>}
                  </View>
                )}
              </>
              ...
              <View style={sum.predCard}>
                ...
                <MathText style={sum.predPrompt}>{(slide.prompt)}</MathText>
                <View style={sum.predHintBox}>
                  <Text style={sum.predHintLabel}>Respuesta</Text>
                  <MathText style={sum.predHint}>{(slide.hint)}</MathText>
                </View>
              </View>
            ...
            // ── Mission model screens ──────────────────────────────
                ...
                return (
                  <View style={sum.missionCard}>
                    <View style={[sum.missionGrad, { backgroundColor: BRAND }]}>
                      ...
                      <MathText style={sum.missionTitle}>{(slide.title)}</MathText>
                          ...
                          {slide.connector!.split('↓').map((part, i) => {
                            ...
                            return i % 2 === 0 ? (
                              <View key={i} style={sum.chainNode}>
                                <MathText style={sum.chainNodeText}>{(text)}</MathText>
                              </View>
                            ) : (
                              <View key={i} style={sum.chainLink}>
                                <Text style={sum.chainLinkArrow}>↓</Text>
                                <MathText style={sum.chainLinkText}>{(text)}</MathText>
                              </View>
                            );
                          })}
                        </View>
                          ...
                          {defLines.map((line, i) => {
                            ...
                            return (
                              <View key={i} style={[sum.stepRow, isResult && sum.stepRowResult, isProblem && sum.stepRowProblem]}>
                                ...
                                <MathText style={[sum.stepContent, isResult && sum.stepContentResult]}>{(content)}</MathText>
                              </View>
                            );
                          })}
                        </View>
                      ...
                      <>
                        ...
                        {showFormalDef && (
                          <View style={sum.formalDefBox}>
                            <MathText style={sum.formalDefText}>{slide.formalDefinition}</MathText>
                          </View>
                        )}
                      </>
                    )}
                ...
                <ScrollView style={{ flex: 1 }} contentContainerStyle={{ flexGrow: 1, paddingBottom: 24 }} showsVerticalScrollIndicator={false}>
                  ...
                  {exercisePrompt ? (
                    <View style={sum.weiCardWrap}>
                      <Image
                        source={require('@/assets/images/conLapiz.png')}
                        style={sum.weiMascot}
                        resizeMode="contain"
                        // pointerEvents isn't in this RN version's ImageProps
                        // type (a plain prop causes a TS2769 overload error)
                        // — spread-with-any bypasses the excess-property
                        // check without affecting the absolute positioning a
                        // wrapping <View pointerEvents="none"> would (an
                        // extra non-positioned parent would make weiMascot's
                        // position:'absolute' resolve against IT instead of
                        // weiCardWrap).
                        {...({ pointerEvents: 'none' } as any)}
                      />
              ...
              // Desafío's worked_example rendering. Still fully PASSIVE —
              // steps render read-only in the order slide.steps already
              // gives, no drag/reorder/validation (that's a separate future
              // phase, reusing order_sequence's interaction if it happens).
              <ScrollView style={{ flex: 1 }} contentContainerStyle={{ flexGrow: 1, paddingBottom: 24 }} showsVerticalScrollIndicator={false}>
                <View style={sum.weHeaderWrap}>
                  ...
                  <Text style={sum.weSubtitle}>Sigue cada paso para llegar a la solución.</Text>
                  <Image
                    source={require('@/assets/images/iluminado.png')}
                    style={sum.weMascotImg}
                    resizeMode="contain"
                    // pointerEvents isn't in this RN version's ImageProps type;
                    // spread-with-any bypasses the excess-property check.
                    {...({ pointerEvents: 'none' } as any)}
                  />
                </View>
                  ...
                  <View style={sum.weStepsContainer}>
                    {slide.steps.map((step, i) => (
                      <View key={i} style={sum.weStepCard}>
                        <View style={[sum.weStepCircle, { backgroundColor: MATCH_ROW_COLORS[i % MATCH_ROW_COLORS.length] }]}>
                          <Text style={sum.weStepCircleText}>{i + 1}</Text>
                        </View>
                        <MathText style={sum.weStepContent}>{(step)}</MathText>
                      </View>
                    ))}
                  </View>
                )}
              ...
              // Always multiple choice (assemble.ts builds options/correctAnswer
              // via shuffleWithLetterAnswer, same as every other MC slide) —
              // no tap-to-reveal, no pre-answer spoiler. Neutral presentation:
              // the problem and the student's attempt are shown side by side
              // with no red/error tinting on either — the student judges which
              // diagnosis is right, they aren't told upfront. Options-list JSX
              // below is the same block micro_challenge/reinforcement_challenge
              // use, so acierto/error feedback and requeue-on-wrong behave
              // identically. Only after answering (fbActive, in the shared
              // bottom bar) does this branch's own "LO CORRECTO ES" box reveal
              // the real correctStep.
              (() => {
                const answered = quizAnswers[summaryIdx];
                return (
                  <ScrollView style={{ flex: 1 }} contentContainerStyle={{ flexGrow: 1, paddingBottom: 24 }} showsVerticalScrollIndicator={false}>
                    <View style={sum.findErrorProblemBox}>
                      <Text style={sum.findErrorProblemLabel}>El problema:</Text>
                      <MathText style={sum.findErrorProblemText}>{(slide.expression)}</MathText>
                    </View>
                    ...
                    {slide.options?.length ? (
                      <View key={`options-${summaryIdx}-${answered ?? 'none'}`} style={{ gap: 12, marginTop: 14 }}>
                          ...
                          return (
                            <Animated.View key={i} style={[wrongShakeStyle, optAnimStyles[i], optEnterStyles[i], showGreen && correctGlowStyle]}>
                              ...
                              >
                                ...
                                <MathText style={[sum.quizOptText, showGreen && { color: paletteExtras.verdeTextoOscuro, fontWeight: '700' }, showRed && { color: palette.rojoErrorDark, fontWeight: '700' }]}>{opt}</MathText>
                              </Pressable>
                            </Animated.View>
                          );
                        })}
                    ...
                    {!!answered && (
                      <View style={[sum.weResultBox, { marginTop: 16 }]}>
                        <View style={sum.weResultCheck}>
                          <Check size={16} color={palette.blanco} strokeWidth={3} />
                        </View>
                        <View style={{ flex: 1 }}>
                  ...
                  <>
                    {!!slide.example && (
                      <View style={sum.comprehensionCtx}>
                        <MathText style={sum.comprehensionCtxText}>{(slide.example)}</MathText>
                      </View>
                    )}
                    <MathText style={sum.quizQuestionFull}>{slide.question ?? slide.title}</MathText>
                      <View key={`options-${summaryIdx}-${quizAnswers[summaryIdx] ?? 'none'}`} style={{ gap: 12, marginTop: 14 }}>
                        {slide.options?.map((opt, i) => {
                          const letter    = LETTERS[i];
                          const answered  = quizAnswers[summaryIdx];
              ...
              <View style={sum.processCard}>
                ...
                <MathText style={sum.processTitle}>{(slide.title)}</MathText>
                {!!slide.definition && (
                  <View style={sum.processSteps}>
                    {slide.definition.split('→').map((step, i) => (
                      <View key={i} style={sum.processStep}>
                        ...
                        <MathText style={sum.processStepText}>{(step.replace(/^\d+\.\s*/, '').trim())}</MathText>
                      </View>
                    ))}
                  </View>
                )}
              ...
              <View style={sum.appCard}>
                ...
                <View style={sum.appBody}>
                  <MathText style={sum.appTitle}>{(slide.title)}</MathText>
                  ...
                  {!!slide.example && (
                    <View style={sum.appScenarioBox}>
                      ...
                      <MathText style={sum.appScenarioText}>{(slide.example)}</MathText>
                    </View>
                  )}
                  {/* Definition as secondary context — only shown if adds new info */}
                  {!!slide.definition && <MathText style={sum.appSit}>{(slide.definition)}</MathText>}
                  {/* Interactive question */}
                  {slide.question && slide.options?.length ? (
                    <View key={`options-${summaryIdx}-${quizAnswers[summaryIdx] ?? 'none'}`} style={{ marginTop: 10, gap: 12 }}>
                      <MathText style={sum.quizQuestion}>{(slide.question)}</MathText>
                      {slide.options.map((opt, i) => {
                        const letter = LETTERS[i];
                        const answered = quizAnswers[summaryIdx];
                      ...
                      {!!quizAnswers[summaryIdx] && (
                        <View style={[sum.quizFeedback, quizAnswers[summaryIdx] === (slide as BackendSlide).correctAnswer ? sum.quizFeedbackOk : sum.quizFeedbackErr]}>
                          {quizAnswers[summaryIdx] === (slide as BackendSlide).correctAnswer ? (
                            ...
                            <>
                              <Text style={sum.quizFeedbackTitle}>💡 Incorrecto</Text>
                              <MathText style={sum.quizFeedbackText}>{(slide.definition) || `La respuesta correcta era la ${(slide as BackendSlide).correctAnswer}.`}</MathText>
                            </>
                          )}
                        </View>
                      )}
              ...
              slide.question && slide.options?.length ? (
                  ...
                  <View style={sum.errorBody}>
                    {!!slide.definition && (
                      <View style={sum.errorWrongBox}>
                        ...
                        <MathText style={sum.errorWrongText}>{(slide.definition)}</MathText>
                      </View>
                    )}
                    <MathText style={[sum.quizQuestion, { marginTop: 10 }]}>{(slide.question)}</MathText>
                    ...
                    {!!quizAnswers[summaryIdx] && !!slide.example && (
                      <View style={[sum.errorRightBox, { marginTop: 10 }]}>
                        ...
                        <MathText style={sum.errorRightText}>{(slide.example)}</MathText>
                      </View>
                    )}
                  </View>
                </View>
                ...
                <View style={sum.introCard}>
                  ...
                  <MathText style={sum.introHeading}>{(slide.title)}</MathText>
                  {!!(slide.definition || slide.example) && (
                    <MathText style={sum.introDef}>{(slide.definition || slide.example)}</MathText>
                  )}
                </View>
              ) : (
                <View style={sum.errorCard}>
                      ...
                      {shuffled.map((origIdx, displayPos) => {
                        ...
                        return (
                          ...
                          >
                            ...
                            <MathText style={[sum.orderItemTxt, isSelected && sum.orderItemTxtSelected]}>
                              {(opts[origIdx])}
                            </MathText>
                          </Pressable>
                        );
              ...
              <View style={sum.challengeRefCard}>
                ...
                {!!slide.definition && <MathText style={sum.challengeRefQ}>{(slide.definition)}</MathText>}
                <View key={`options-${summaryIdx}-${quizAnswers[summaryIdx] ?? 'none'}`} style={{ gap: 12, marginTop: 10, width: '100%' }}>
                  ...
                  {!!quizAnswers[summaryIdx] && !!slide.example && (
                    <View style={sum.challengeRefHintBox}>
                      ...
                      <MathText style={sum.challengeRefHintTxt}>{(slide.example)}</MathText>
                    </View>
                  )}
                </View>
              </View>
            ...
            // ── Legacy screens ────────────────────────────────────
              ...
              <View style={sum.introCard}>
                ...
                <MathText style={sum.introHeading}>{(slide.title)}</MathText>
                {!!slide.definition && <MathText style={sum.introDef}>{(slide.definition)}</MathText>}
                {!!slide.example && (
                  <View style={sum.exampleBox}>
                    <Text style={sum.exampleLabel}>📌 Ejemplo</Text>
                    <MathText style={sum.exampleText}>{slide.example}</MathText>
              ...
              <View style={sum.wowCard}>
                ...
                {!!slide.title && <MathText style={sum.wowHook}>{(slide.title)}</MathText>}
                {/* Definition expands the surprising fact */}
                <View style={sum.wowDataBox}>
                  <MathText style={sum.wowText}>{(slide.definition)}</MathText>
                </View>
                {/* Example as "¿por qué importa?" — only when no question */}
                {!!slide.example && !slide.question && (
                  <MathText style={sum.wowContext}>{(slide.example)}</MathText>
                )}
                {slide.question && slide.options && (
                  <View key={`options-${summaryIdx}-${quizAnswers[summaryIdx] ?? 'none'}`} style={{ gap: 12, marginTop: 16, alignSelf: 'stretch' }}>
                    <MathText style={[sum.quizQuestion, { fontSize: SM ? 13 : 14 }]}>{(slide.question)}</MathText>
                    {slide.options.map((opt, i) => {
                      const letter    = LETTERS[i];
                      const answered  = quizAnswers[summaryIdx];
                      const isCorrect = slide.correctAnswer === letter;
                    ...
                    {!!quizAnswers[summaryIdx] && (
                      <View style={[sum.quizFeedback, quizAnswers[summaryIdx] === slide.correctAnswer ? sum.quizFeedbackOk : sum.quizFeedbackErr]}>
                        {quizAnswers[summaryIdx] === slide.correctAnswer ? (
                          ...
                          <>
                            <Text style={sum.quizFeedbackTitle}>💡 Incorrecto</Text>
                            {!!slide.example && <MathText style={sum.quizFeedbackText}>{(slide.example)}</MathText>}
                          </>
                        )}
                      </View>
                    )}
              ...
              <View style={sum.scenarioCard}>
                ...
                <View style={sum.scenarioBody}>
                  <MathText style={sum.scenarioTitle}>{(slide.title)}</MathText>
                  {!!slide.definition && <MathText style={sum.scenarioDef}>{(slide.definition)}</MathText>}
                  {!!slide.example && <MathText style={sum.scenarioEx}>{(slide.example)}</MathText>}
                </View>
              </View>
              ...
              <View style={[sum.kpCard, { backgroundColor: SLIDE_STYLE[slide?.type ?? '']?.bg ?? 'rgba(22,119,242,0.08)', borderLeftColor: SLIDE_STYLE[slide?.type ?? '']?.accent ?? BRAND }]}>
                ...
                <MathText style={sum.kpTitle}>{(slide?.title)}</MathText>
                {!!slide?.definition && <MathText style={sum.kpDef}>{(slide.definition)}</MathText>}
                {!!slide?.example && (
                  <View style={sum.exampleBox}>
                    <Text style={sum.exampleLabel}>📌 Ejemplo</Text>
                    <MathText style={sum.exampleText}>{slide.example}</MathText>
          ...
          {/* CTA — always a plain <View> at this SafeAreaView position.
              Keeping the same component type prevents Fabric from issuing a
          ...
          {safeRender(() => {
            ...
            const MISSION_QUIZ_TYPES = new Set(['micro_challenge', 'reinforcement_challenge', 'comprehension', 'mini_quiz', 'final_challenge', 'decide', 'find_error']);
            ...
            const isConceptGated = slide?.type === 'main_concept'
              ...
              && !conceptRevealed;
            // find_error is now a plain MC slide (see its render branch) —
            // joins MISSION_QUIZ_TYPES above, so it's already gated by
            // isMissionInteractive like micro_challenge/reinforcement_challenge;
            // no bespoke gate needed here anymore.
            ...
            return (
              <View style={fbActive
                ? [sum.mFeedbackBar, missionCorrect ? sum.mFeedbackBarOk : sum.mFeedbackBarErr,
          ...
          <ScrollView contentContainerStyle={[qz.scroll, { paddingBottom: 8 }]} showsVerticalScrollIndicator={false}>
            ...
            <Animated.View style={questionTransStyle}>
              <View style={qz.questionCard}>
                ...
                <MathText style={qz.questionText}>{(question?.text)}</MathText>
              </View>
...
const mds = StyleSheet.create(withMisionFont({
  ...
  desafioCard:    { backgroundColor: palette.charcoal, borderRadius: 20, padding: SM ? 16 : 20 },
...
const sum = StyleSheet.create(withMisionFont({
  ...
  weiTitle:       { fontSize: SM ? 26 : 30, fontWeight: '800', color: semantic.textPrimary, lineHeight: SM ? 30 : 34, maxWidth: '62%' },
  weiUnderline:   { width: 130, height: 6, borderRadius: 3, backgroundColor: palette.amarilloXP, marginTop: 4, marginBottom: 10 },
  weiSubtitle:    { fontSize: 14, color: semantic.textSecondary, lineHeight: 20, maxWidth: '55%' },
  ...
  // Post-answer mascot feedback — micro_challenge/reinforcement_challenge/
  // final_challenge, which previously only recolored the tapped option.
  feedbackRow:            { flexDirection: 'row', alignItems: 'flex-start', gap: 8, marginTop: 16 },
  feedbackMascot:         { width: 34, height: 34, marginTop: 2 },
  feedbackBubble:         { flex: 1, borderRadius: 14, paddingVertical: 10, paddingHorizontal: 13, position: 'relative' as const, borderWidth: 1 },
  feedbackBubbleCorrect:  { backgroundColor: paletteExtras.verdeSuaveBg2, borderColor: paletteExtras.verdeChipBorde },
  feedbackBubbleWrong:    { backgroundColor: palette.rojoErrorBg, borderColor: 'rgba(220,38,38,0.25)' },
  feedbackBubbleTail:     { position: 'absolute' as const, left: -5, top: 12, width: 10, height: 10, transform: [{ rotate: '45deg' }] },
  feedbackBubbleTailCorrect: { backgroundColor: paletteExtras.verdeSuaveBg2 },
  feedbackBubbleTailWrong:   { backgroundColor: palette.rojoErrorBg },
  feedbackTitleCorrect:   { fontSize: 13, fontWeight: '800' as const, color: paletteExtras.verdeTextoOscuro },
  feedbackTitleWrong:     { fontSize: 13, fontWeight: '800' as const, color: palette.rojoErrorDark, marginBottom: 3 },
  feedbackText:           { fontSize: 12.5, color: palette.rojoErrorDark, lineHeight: 18, fontWeight: '500' as const },
  ...
  weSubtitle:      { fontSize: 13, color: semantic.textSecondary, lineHeight: 18, maxWidth: '58%' },

  // Mascot + bubble — same visual language as match_pairs' mpMascot* (own
  // "we"-prefixed copy so this branch's tuning never touches that one).
  weMascotRow:            { position: 'relative', overflow: 'visible', marginBottom: 16, zIndex: 5 },
  weMascotBubble:         { backgroundColor: palette.blanco, borderRadius: 14, borderWidth: 1, borderColor: palette.bordeClaro, paddingVertical: 8, paddingHorizontal: 11, alignSelf: 'flex-start', maxWidth: '62%', paddingRight: 12, position: 'relative' },
  ...
  // paletteExtras "morado*" tokens — those were migrated to blue during the
  ...
  // find_error (capabilities.findError) — NEUTRAL presentation on purpose:
  // no red/error tinting on either box, since the point is for the student
  // to JUDGE whether the resolution is right, not be shown a spoiler before
  // answering. Same plain card shape for both "problem" and "attempt",
  // distinguished only by label text. weResultBox (green, pre-existing) is
  // reused as-is for the post-answer "LO CORRECTO ES" reveal — that one
  // legitimately shows resolved state, since it only renders after the
  ...
  // bottom CTA bar.
  weCheerBar:   { flexDirection: 'row', alignItems: 'center', gap: 10, backgroundColor: paletteExtras.verdeSuaveBg2, borderRadius: 14, padding: 12 },
  weCheerEmoji: { fontSize: 18 },
  weCheerText:  { flex: 1, fontSize: 13, color: paletteExtras.verdeTextoOscuro, lineHeight: 18, fontWeight: '700' },
